...
def getFlags(myenum, flags):
    return "|".join(
        [name for name, member in myenum.__members__.items() if member.value & flags]
    )
...
class MasterKey(Structure):
    ...
    def dump(self):
        ...
        print(
            "Rounds      : %8x (%d)"
            % (self["MasterKeyIterationCount"], self["MasterKeyIterationCount"])
        )
        print(
            "HashAlgo    : %.8x (%d) (%s)"
    ...
    def deriveKey(self, passphrase, salt, keylen, count, hashFunction):
        ...
        while len(keyMaterial) < keylen:
            ...
            for r in range(count - 1):
                ...
                if PY3:
                    derived = (
                        int.from_bytes(derived, sys.byteorder)
                        ^ int.from_bytes(actual, sys.byteorder)
                    ).to_bytes(len(actual), sys.byteorder)
                else:
                    derived = bytearray(
                        [chr((a) ^ (b)) for (a, b) in zip(derived, actual)]
                    )
            keyMaterial += derived
    ...
    def decrypt(self, key):
        ...
        derivedBlob = self.deriveKey(
            key,
            self["Salt"],
            ALGORITHMS_DATA[self["CryptAlgo"]][0]
            + ALGORITHMS_DATA[self["CryptAlgo"]][3],
            count=self["MasterKeyIterationCount"],
            hashFunction=prf,
        )

        cryptKey = derivedBlob[: ALGORITHMS_DATA[self["CryptAlgo"]][0]]
        iv = derivedBlob[ALGORITHMS_DATA[self["CryptAlgo"]][0] :][
            : ALGORITHMS_DATA[self["CryptAlgo"]][3]
        ]

        cipher = ALGORITHMS_DATA[self["CryptAlgo"]][1].new(
            cryptKey, mode=ALGORITHMS_DATA[self["CryptAlgo"]][2], iv=iv
        )
...
class DPAPI_BLOB(Structure):
    ...
    def deriveKey(self, sessionKey):
        ...
        if len(sessionKey) > ALGORITHMS_DATA[self["HashAlgo"]][4]:
            derivedKey = HMAC.new(
                sessionKey, digestmod=ALGORITHMS_DATA[self["HashAlgo"]][1]
            ).digest()
        else:
        ...
        if len(derivedKey) < ALGORITHMS_DATA[self["CryptAlgo"]][0]:
            ...
            ipad = bytearray(
                [i ^ 0x36 for i in bytearray(derivedKey)][
                    : ALGORITHMS_DATA[self["HashAlgo"]][4]
                ]
            )
            opad = bytearray(
                [i ^ 0x5C for i in bytearray(derivedKey)][
                    : ALGORITHMS_DATA[self["HashAlgo"]][4]
                ]
            )
            derivedKey = (
    ...
    def decrypt(self, key, entropy=None):
        ...
        sessionKey = HMAC.new(
            keyHash, self["Salt"], ALGORITHMS_DATA[self["HashAlgo"]][1]
        )
        ...
        ipad = bytearray(
            [i ^ 0x36 for i in bytearray(keyHash2)][
                : ALGORITHMS_DATA[self["HashAlgo"]][1].block_size
            ]
        )
        opad = bytearray(
            [i ^ 0x5C for i in bytearray(keyHash2)][
                : ALGORITHMS_DATA[self["HashAlgo"]][1].block_size
            ]
        )
        ...
        hmacCalculated3 = HMAC.new(
            keyHash, self["HMac"], ALGORITHMS_DATA[self["HashAlgo"]][1]
        )
        if entropy is not None:
        ...
        if (
            hmacCalculated1.digest() == self["Sign"]
...
class VAULT_ATTRIBUTE(Structure):
    ...
    extended = (
        ...
        (
            "_Data",
            "_-Data",
            'self["Size"]-self["IVSize"]-5 if self["IVPresent"] else self["Size"]-1',
        ),
        ("Data", ":"),
    )
...
class VAULT_VCRD(Structure):
    ...
    def __init__(self, data=None, alignment=0):
        ...
        if data is not None:
            ...
            for i in range(
                self["AttributesMapsSize"] // len(VAULT_ATTRIBUTE_MAP_ENTRY())
            ):
            ...
            for i in range(len(self.mapEntries)):
                if i > 0:
                    self.attributesLen.append(
                        self.mapEntries[i]["Offset"] - self.mapEntries[i - 1]["Offset"]
                    )
            ...
            self["Data"] = self.rawData[
                self.mapEntries[-1]["Offset"] + len(self.attributes[-1].getData()) :
            ]
...
class VAULT_WIN_BIO_KEY(Structure):
    ...
    def dump(self):
        ...
        print(
            "Sid          : %s"
            % RPC_SID(b"\x05\x00\x00\x00" + self["Sid"]).formatCanonical()
        )
        print("Friendly Name: %s" % self["Name"].decode("utf-16le"))
        print(
            "Biometric Key: 0x%s" % (hexlify(self["BioKey"]["bKey"])).decode("latin-1")
        )
        print()
