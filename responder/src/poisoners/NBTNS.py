...
import sys
from responder.src.packets import NBT_Ans
            ...
            if data[2:4] == b"\x01\x10":
                if settings.Config.AnalyzeMode:  # Analyze Mode
                    ...
                    if not settings.Config.Quiet_Mode:
                        print(
                            text(
                                "[Analyze mode: NBT-NS] Request by %-15s for %s, ignoring"
                                % (
                                    color(
                                        self.client_address[0].replace("::ffff:", ""), 3
                                    ),
                                    color(Name, 3),
                                )
                            )
                        ...
                        print(
                            color(
                                ...
                                % (
                                    ...
                                    Name,
                                    NBT_NS_Role(
                                        NetworkRecvBufferPython2or3(data[43:46])
                                    ),
                                ),
                                2,
                                1,
