...
def hBaseRegCreateKey(
    dce,
    hKey,
    lpSubKey,
    lpClass=NULL,
    dwOptions=0x00000001,
...
def hBaseRegEnumKey(dce, hKey, dwIndex, lpftLastWriteTime=NULL):
    ...
    request.fields["lpNameIn"].fields["Data"].fields["Data"].fields["MaximumCount"] = (
        1024 // 2
    )
...
def hBaseRegEnumValue(dce, hKey, dwIndex, dataLen=256):
    ...
    while True:
        try:
            ...
            request.fields["lpValueNameIn"].fields["Data"].fields["Data"].fields[
                "MaximumCount"
            ] = dataLen
...
def hBaseRegOpenKey(
    dce, hKey, lpSubKey, dwOptions=0x00000001, samDesired=MAXIMUM_ALLOWED
):
...
def hBaseRegQueryInfoKey(dce, hKey):
    ...
    request.fields["lpClassIn"].fields["Data"].fields["Data"].fields["MaximumCount"] = (
        1024 // 2
    )
    return dce.request(request)
...
def hBaseRegQueryMultipleValues(dce, hKey, val_listIn):
    ...
    for item in resp["val_listOut"]:
        ...
        itemn["ValueData"] = unpackValue(
            item["ve_type"],
            resp["lpvalueBuf"][
                item["ve_valueptr"] : item["ve_valueptr"] + item["ve_valuelen"]
            ],
        )
        retVal.append(itemn)
