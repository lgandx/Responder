...
RPC_PROXY_REMOTE_NAME_NEEDED_ERR = (
    "Basic authentication in RPC proxy is used, "
...
################################################################################
def hCONN_A1(
    virtualConnectionCookie=EMPTY_UUID,
    outChannelCookie=EMPTY_UUID,
...
def hFlowControlAckWithDestination(
    destination, bytesReceived, availableWindow, channelCookie
):
...
class RPCProxyClient(HTTPClientSecurityProvider):
    ...
    def set_proxy_credentials(
        self, username, password, domain="", lmhash="", nthash=""
    ):
        LOG.error(
            "DeprecationWarning: Call to deprecated method set_proxy_credentials (use set_credentials)."
        )
    ...
    def set_credentials(
        ...
        HTTPClientSecurityProvider.set_credentials(
            self, username, password, domain, lmhash, nthash, aesKey, TGT, TGS
        )
    ...
    def create_channel(self, method, headers):
        self.__channels[method] = HTTPClientSecurityProvider.connect(
            self, self._rpcProxyUrl.scheme, self._rpcProxyUrl.netloc
        )

        auth_headers = HTTPClientSecurityProvider.get_auth_headers(
            self, self.__channels[method], method, self._rpcProxyUrl.path, headers
        )[0]
        ...
        if resp[9:23] != b"100 Continue\r\n":
            try:
                ...
                raise RPCProxyClientException(
                    "RPC Proxy Client: %s authentication failed in %s channel"
    ...
    def create_tunnel(self):
        ...
        packet = hCONN_A1(
            self.__virtualConnectionCookie,
            self.__outChannelCookie,
            self.__availableWindowAdvertised,
        )
        self.get_socket_out().send(packet)

        packet = hCONN_B1(
            self.__virtualConnectionCookie,
            self.__inChannelCookie,
            self.__associationGroupId,
        )
        ...
        if resp[9:12] != b"200":
            try:
                ...
                raise RPCProxyClientException(
                    "RPC Proxy CONN/A1 request failed", proxy_error=resp
                )
            except (IndexError, KeyError, AttributeError):
        ...
        self.__serverConnectionTimeout = conn_a3["ConnectionTimeout"][
            "ConnectionTimeout"
        ]
        ...
        self.__serverReceiveWindowSize = conn_c2["ReceiveWindowSize"][
            "ReceiveWindowSize"
        ]
    ...
    def check_http_error(self, buffer):
        if buffer[:22] == b"HTTP/1.0 503 RPC Error":
            raise RPCProxyClientException(
                "RPC Proxy request failed", proxy_error=buffer
            )
    ...
    def rpc_out_read_pkt(self, handle_rts=False):
        while True:
            ...
            while len(response_data) < MSRPCHeader._SIZE:
                response_data += self.rpc_out_recv1(
                    MSRPCHeader._SIZE - len(response_data)
                )
    ...
    def flow_control(self, frag_len):
        ...
        if self.__receiverAvailableWindow < self.__availableWindowAdvertised // 2:
            self.__receiverAvailableWindow = self.__availableWindowAdvertised
            packet = hFlowControlAckWithDestination(
                FDOutProxy,
