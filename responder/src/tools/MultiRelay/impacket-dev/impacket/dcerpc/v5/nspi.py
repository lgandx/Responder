...
class DCERPCSessionError(DCERPCException):
    def __str__(self):
        ...
        if key in mapi_constants.ERROR_MESSAGES:
            ...
            return "NSPI SessionError: code: 0x%x - %s" % (
                self.error_code,
                error_msg_short,
            )
...
def simplifyPropertyRow(rowSetElem):
    ...
    for prop in rowSetElem["lpProps"]:
        ...
        if (
            isinstance(prop_value, SHORT)
            or isinstance(prop_value, USHORT)
            or isinstance(prop_value, LONG)
            or isinstance(prop_value, ULONG)
        ):
            ...
            if PropTag in [0x80270102, 0x8C750102]:
                ...
                value[4:20] == GUID_NSPI
                and value[20:24] == b"\x01\x00\x00\x00"
                and value[:4] == b"\x00\x00\x00\x00"
            ):
                value = PermanentEntryID(value)
            elif (
                value[:4] == b"\x87\x00\x00\x00"
                and value[20:24] == b"\x01\x00\x00\x00"
                and len(value) == 32
            ):
...
def hNspiQueryRows(
    ...
    if len(pPropTags) > 0:
        ...
        request.fields["pPropTags"].fields["Data"].fields["aulPropTag"].fields[
            "MaximumCount"
        ] = (len(pPropTags) + 1)
    else:
...
def hNspiSeekEntries(
    ...
    if len(pPropTags) > 0:
        for aulPropTag in pPropTags:
            ...
            prop["Data"] = aulPropTag
            request["pPropTags"]["aulPropTag"].append(prop)
        request.fields["pPropTags"].fields["aulPropTag"].fields["MaximumCount"] = (
            len(pPropTags) + 1
        )
    else:
        request["pPropTags"] = pPropTagsRaw
...
def hNspiGetSpecialTable(
    dce, handler, dwFlags=NspiUnicodeStrings, pStat=STAT(), lpVersion=NULL
):
...
def hNspiGetTemplateInfo(
    dce, handler, pDN=NULL, dwLocaleID=0, ulType=0, dwCodePage=0, dwFlags=0xFFFFFFFF
):
...
def hNspiGetNamesFromIDs(
    dce, handler, lpguid=EMPTY_UUID, pPropTags=[], pPropTagsRaw=NULL
):
...
def hNspiResolveNames(
    dce, handler, ContainerID=0, pPropTags=[], pPropTagsRaw=NULL, paStr=[]
):
