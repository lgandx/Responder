...
class DCERPCSessionError(DCERPCException):
    ...
    def __str__(self):
        if self.error_code in hresult_errors.ERROR_MESSAGES:
            ...
            return "WMI SessionError: code: 0x%x - %s - %s" % (
                self.error_code,
                error_msg_short,
                error_msg_verbose,
            )
            ...
            try:
                return "WMI Session Error: code: 0x%x - %s" % (
                    self.error_code,
                    WBEMSTATUS.enumItems(self.error_code).name,
                )
            except:
...
class ENCODED_STRING(Structure):
    commonHdr = (("Encoded_String_Flag", ENCODED_STRING_FLAG),)

    tascii = (("Character", "z"),)

    tunicode = (("Character", "u"),)
...
class ENCODED_VALUE(Structure):
    structure = (("QualifierName", QUALIFIER_NAME),)
            ...
            if cimType & CIM_ARRAY_FLAG:
                ...
                if cimType == CIM_TYPE_ENUM.CIM_ARRAY_STRING.value:
                    ...
                    for item in range(numItems):
                        ...
                        array.append(
                            unpack(unpackStrArray, heapData[:dataSizeArray])[0]
                        )
    ...
    def __init__(self, data=None, alignment=0):
        ...
        if data is not None:
            ...
            self.structure = (
                (
                    "QualifierValue",
                    CIM_TYPES_REF[self["QualifierType"] & (~CIM_ARRAY_FLAG)],
                ),
            )
            self.fromString(data)
        else:
    ...
    def getProperties(self, heap):
        ...
        for property in range(self["PropertyCount"]):
            ...
            if propItem["PropertyNameRef"] & 0x80000000:
                propName = DICTIONARY_REFERENCE[
                    propItem["PropertyNameRef"] & 0x7FFFFFFF
                ]
            else:
                propName = ENCODED_STRING(heap[propItem["PropertyNameRef"] :])[
                    "Character"
                ]
            ...
            while len(qualifiersBuf) > 0:
                ...
                if record["QualifierName"] & 0x80000000:
                    ...
                    qualifierName = ENCODED_STRING(heap[record["QualifierName"] :])[
                        "Character"
                    ]
...
class CLASS_PART(Structure):
    ...
    def getProperties(self):
        ...
        properties = self["PropertyLookupTable"].getProperties(
            self["ClassHeap"]["HeapItem"]
        )
        sorted_props = sorted(
    ...
    def getMethods(self):
        ...
        for method in range(self["MethodCount"]):
            ...
            if itemn["MethodFlags"] & WBEM_FLAVOR_ORIGIN_PROPAGATED:
                ...
                # raise
                pass
            methodDict["name"] = ENCODED_STRING(heap[itemn["MethodName"] :])[
                "Character"
            ]
            ...
            if itemn["InputSignature"] != 0xFFFFFFFF:
                ...
                if inputSignature["EncodingLength"] > 0:
                    methodDict["InParams"] = inputSignature["ObjectBlock"]["ClassType"][
                        "CurrentClass"
                    ].getProperties()
            ...
            if itemn["OutputSignature"] != 0xFFFFFFFF:
                outputSignature = METHOD_SIGNATURE_BLOCK(
                    heap[itemn["OutputSignature"] :]
                )
                if outputSignature["EncodingLength"] > 0:
                    methodDict["OutParams"] = outputSignature["ObjectBlock"][
                        "ClassType"
                    ]["CurrentClass"].getProperties()
        ...
        if pClassName == 0xFFFFFFFF:
            ...
            while len(derivationList) > 0:
                ...
                derivationList = derivationList[
                    len(ENCODED_STRING(derivationList)) + 4 :
                ]
            return className
...
class CURRENT_CLASS_NO_METHODS(CLASS_AND_METHODS_PART):
    structure = (("ClassPart", ":", CLASS_PART),)
...
class INSTANCE_PROP_QUALIFIER_SET(Structure):
    commonHdr = (("InstPropQualSetFlag", INST_PROP_QUAL_SET_FLAG),)
    tail = (
...
class INSTANCE_TYPE(Structure):
    ...
    def __init__(self, data=None, alignment=0):
        ...
        if data is not None:
            ...
            self.NdTableSize = (
                self["CurrentClass"]["ClassPart"]["PropertyLookupTable"][
                    "PropertyCount"
                ]
                - 1
            ) // 4 + 1
...
class OBJECT_BLOCK(Structure):
    commonHdr = (("ObjectFlags", OBJECT_FLAGS),)
...
class METHOD_SIGNATURE_BLOCK(Structure):
    commonHdr = (("EncodingLength", ENCODING_LENGTH),)
    tail = (
...
IID_IWbemBackupRestoreEx = uuidtup_to_bin(
    ("A359DEC5-E813-4834-8A2A-BA7F1D777D76", "0.0")
)
...
class PWBEM_REFRESHED_OBJECT_ARRAY(NDRPOINTER):
    referent = (("Data", WBEM_REFRESHED_OBJECT_ARRAY),)
...
class IWbemServices_CancelAsyncCall(DCOMCALL):
    opnum = 4
    structure = (("IWbemObjectSink", PMInterfacePointer),)
...
class IUnsecuredApartment_CreateObjectStub(DCOMCALL):
    opnum = 3
    structure = (("reserved1", PMInterfacePointer),)
...
class IWbemClassObject(IRemUnknown):
    ...
    def getClassName(self):
        if self.encodingUnit["ObjectBlock"].isInstance() is False:
            return (
                self.encodingUnit["ObjectBlock"]["ClassType"]["CurrentClass"]
                .getClassName()
                .split(" ")[0]
            )
        else:
            return (
                self.encodingUnit["ObjectBlock"]["InstanceType"]["CurrentClass"]
                .getClassName()
                .split(" ")[0]
            )
    ...
    def marshalMe(self):
        ...
        encodingUnit["ObjectBlock"]["InstanceType"]["NdTable_ValueTable"] = (
            packedNdTable + valueTable
        )
        encodingUnit["ObjectBlock"]["InstanceType"]["InstanceHeap"]["HeapLength"] = (
            len(instanceHeap) | 0x80000000
        )
        encodingUnit["ObjectBlock"]["InstanceType"]["InstanceHeap"][
            "HeapItem"
        ] = instanceHeap

        encodingUnit["ObjectBlock"]["InstanceType"]["EncodingLength"] = len(
            encodingUnit["ObjectBlock"]["InstanceType"]
        )
    ...
    def SpawnInstance(self):
        ...
        if self.encodingUnit["ObjectBlock"].isInstance() is False:
            ...
            instanceData["Decoration"] = self.encodingUnit["ObjectBlock"][
                "Decoration"
            ].getData()
            ...
            instanceType["CurrentClass"] = self.encodingUnit["ObjectBlock"][
                "ClassType"
            ]["CurrentClass"]["ClassPart"]
    ...
    def createProperties(self, properties):
        for property in properties:
            ...
            if properties[property]["type"] == CIM_TYPE_ENUM.CIM_TYPE_OBJECT.value:
                ...
                objRef["ObjectReferenceSize"] = len(
                    properties[property]["value"].getData()
                )
    ...
    def createMethods(self, classOrInstance, methods):
        ...
        def innerMethod(staticArgs, *args):
            ...
            if methodDefinition["InParams"] is not None:
                ...
                for i in range(len(args)):
                    ...
                    if paramDefinition["type"] & CIM_ARRAY_FLAG:
                        if inArg is None:
                            ...
                            for j in range(len(inArg)):
                                ...
                                if pType == CIM_TYPE_ENUM.CIM_TYPE_OBJECT.value:
                                    ...
                                    curObject += pack(
                                        "<L",
                                        marshaledObject["pObjectData"][
                                            "ObjectEncodingLength"
                                        ],
                                    )
                                    curObject += marshaledObject["pObjectData"][
                                        "ObjectBlock"
                                    ].getData()
                            ...
                            valueTable += pack("<L", curHeapPtr)
                            instanceHeap += (
                                arraySize + arrayHeapPtrValues + arrayValueTable
                            )
                            curHeapPtr = len(instanceHeap)
                        else:
                            arraySize = pack(HEAPREF[:-2], len(inArg))
                        ...
                        if inArg is None:
                            ...
                            instanceHeap += pack(
                                "<L",
                                marshaledObject["pObjectData"]["ObjectEncodingLength"],
                            )
                            instanceHeap += marshaledObject["pObjectData"][
                                "ObjectBlock"
                            ].getData()
                            curHeapPtr = len(instanceHeap)
                    else:
                ...
                instanceType["EncodingLength"] = len(instanceType)
                inMethods = methodDefinition["InParamsRaw"]["ClassType"][
                    "CurrentClass"
                ]["ClassPart"]
                inMethods["ClassHeader"]["EncodingLength"] = len(
                    methodDefinition["InParamsRaw"]["ClassType"]["CurrentClass"][
                        "ClassPart"
                    ].getData()
                )
            ...
            instanceType["EncodingLength"] = len(instanceType)
            instanceType["CurrentClass"] = methodDefinition["OutParamsRaw"][
                "ClassType"
            ]["CurrentClass"]["ClassPart"].getData()
        ...
        for methodName in methods:
            ...
            setattr(
                self,
                innerMethod.__name__,
                innerMethod[classOrInstance, methods[methodName]],
            )
...
class IWbemServices(IRemUnknown):
    ...
    def QueryObjectSink(self):
        ...
        return INTERFACE(
            self.get_cinstance(),
            b"".join(resp["ppResponseHandler"]["abData"]),
            self.get_ipidRemUnknown(),
            target=self.get_target(),
        )
    ...
    def PutInstance(self, pInst, lFlags=0, pCtx=NULL):
        ...
        return IWbemCallResult(
            INTERFACE(
                self.get_cinstance(),
                b"".join(resp["ppCallResult"]["abData"]),
                self.get_ipidRemUnknown(),
                target=self.get_target(),
            )
        )
    ...
    def ExecMethod(
        ...
        return IWbemClassObject(
            INTERFACE(
                self.get_cinstance(),
                b"".join(resp["ppOutParams"]["abData"]),
                self.get_ipidRemUnknown(),
                oxid=self.get_oxid(),
                target=self.get_target(),
            )
    ...
    def ExecMethodAsync(
        self, strObjectPath, strMethodName, lFlags=0, pCtx=NULL, pInParams=NULL
    ):
...
class IWbemLevel1Login(IRemUnknown):
    ...
    def NTLMLogin(self, wszNetworkResource, wszPreferredLocale, pCtx):
        ...
        return IWbemServices(
            INTERFACE(
                self.get_cinstance(),
                b"".join(resp["ppNamespace"]["abData"]),
                self.get_ipidRemUnknown(),
                target=self.get_target(),
            )
        )
