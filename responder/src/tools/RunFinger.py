...
parser.add_option(
    ...
    "--timeout",
    action="store",
    help="Timeout for all connections. Use this option to fine tune Runfinger.",
    dest="Timeout",
    type="float",
    metavar="0.9",
    default=2,
...
SMB1 = "True"
SMB2signing = "False"
DB = settings.LOGS_PATH / "RunFinger.db"
...
if not os.path.exists(DB):
    ...
    cursor.execute(
        "CREATE TABLE RunFinger (timestamp TEXT, Protocol TEXT, Host TEXT, WindowsVersion TEXT, OsVer TEXT, DomainJoined TEXT, Bootime TEXT, Signing TEXT, NullSess TEXT, IsRDPOn TEXT, SMB1 TEXT, MSSQL TEXT)"
    )
    cursor.commit()
    cursor.close()
...
def SaveRunFingerToDb(result):
    ...
    res = cursor.execute(
        "SELECT COUNT(*) AS count FROM RunFinger WHERE Protocol=? AND Host=? AND WindowsVersion=? AND OsVer=? AND DomainJoined=? AND Bootime=? AND Signing=? AND NullSess=? AND IsRDPOn=? AND SMB1=? AND MSSQL=?",
        (
            result["Protocol"],
            result["Host"],
    ...
    if not count:
        cursor.execute(
            "INSERT INTO RunFinger VALUES(datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)",
            (
                result["Protocol"],
                result["Host"],
                result["WindowsVersion"],
...
def ParseSMBNTLM2Exchange(data, host, bootime, signing):  # Parse SMB NTLMSSP Response
    ...
    Domain = SSPIString[TargetNameOffset : TargetNameOffset + TargetNameLen].decode(
        "UTF-16LE"
    )
    ...
    print(
        (
            "[SMB2]:['{}', Os:'{}', Build:'{}', Domain:'{}', Bootime: '{}', Signing:'{}', RDP:'{}', SMB1:'{}', MSSQL:'{}']".format(
                host,
                WindowsVers,
                str(WindowsBuildVers),
                Domain,
                Bootime,
                signing,
                RDP,
...
def OsNameClientVersion(data):
    try:
        ...
        return (
            "Could not fingerprint Os version.",
            "Could not fingerprint LanManager Client version",
        )
...
def SmbFinger(Host):
    ...
    try:
        ...
        if data[8:10] == b"\x73\x16":
            OsVersion, ClientVersion = OsNameClientVersion(
...
def ConnectAndChoseSMB(host):
    ...
    if ParseNegotiateSMB2Ans(data):
        try:
            while True:
                s.send(
                    NetworkSendBufferPython2or3(handle(data.decode("latin-1"), host))
                )
                data = s.recv(4096)
                if not data:
                    break
...
def handle(data, host):
    ...
    if data[28:29] == "\x01":
        ...
        a = SMBv2Head(
            SMBv2Command="\x01\x00", CommandSequence="\x02\x00\x00\x00\x00\x00\x00\x00"
        )
        a.calculate()
        b = SMBv2Session1()
        b.calculate()
...
def ShowSmallResults(Host):
    if ConnectAndChoseSMB((Host, 445)) == False:
        try:
            ...
            print(
                (
                    "[SMB1]:['{}', Os:'{}', Domain:'{}', Signing:'{}', Null Session: '{}', RDP:'{}', MSSQL:'{}']".format(
                        Host, OsVer, DomainJoined, Signing, NullSess, RDP, SQL
                    )
                )
            )
            SaveRunFingerToDb(
