...
IID_IActivationPropertiesIn = uuidtup_to_bin(
    ("000001A2-0000-0000-C000-000000000046", "0.0")
)
IID_IActivationPropertiesOut = uuidtup_to_bin(
    ("000001A3-0000-0000-C000-000000000046", "0.0")
)
...
IID_IRemoteSCMActivator = uuidtup_to_bin(
...
class DCERPCSessionError(DCERPCException):
    ...
    def __str__(self):
        if self.error_code in hresult_errors.ERROR_MESSAGES:
            ...
            return "DCOM SessionError: code: 0x%x - %s - %s" % (
                self.error_code,
                error_msg_short,
                error_msg_verbose,
            )
...
class INTERFACE:
    ...
    def get_dce_rpc(self):
        return INTERFACE.CONNECTIONS[self.__target][currentThread().getName()][
            self.__oxid
        ]["dce"]
    ...
    def connect(self, iid=None):
        if (self.__target in INTERFACE.CONNECTIONS) is True:
            if (
                ...
                and (
                    self.__oxid
                    in INTERFACE.CONNECTIONS[self.__target][currentThread().getName()]
                )
                is True
            ):
                dce = INTERFACE.CONNECTIONS[self.__target][currentThread().getName()][
                    self.__oxid
                ]["dce"]
                currentBinding = INTERFACE.CONNECTIONS[self.__target][
                    currentThread().getName()
                ][self.__oxid]["currentBinding"]
                ...
                LOG.debug(
                    "Target system is %s and isFDQN is %s"
                    % (self.get_target(), isTargetFDQN)
                )
                for strBinding in stringBindings:
                    ...
                    if strBinding["wTowerId"] == 7:
                        ...
                        if strBinding["aNetworkAddr"].find("[") >= 0:
                            binding, _, bindingPort = strBinding[
                                "aNetworkAddr"
                            ].partition("[")
                        ...
                        if binding.upper().find(self.get_target().upper()) >= 0:
                            stringBinding = (
                                "ncacn_ip_tcp:" + strBinding["aNetworkAddr"][:-1]
                            )
                            break
                        # If get_target() is a FQDN, does it match the hostname?
                        elif (
                            isTargetFDQN
                            and binding.upper().find(
                                self.get_target().upper().partition(".")[0]
                            )
                            >= 0
                        ):
                            ...
                            stringBinding = "ncacn_ip_tcp:%s%s" % (
                                self.get_target(),
                                bindingPort,
                            )
                            break
                ...
                if hasattr(dcomInterface, "set_credentials"):
                    # This method exists only for selected protocol sequences.
                    dcomInterface.set_credentials(
                        *DCOMConnection.PORTMAPS[self.__target].get_credentials()
                    )
                    dcomInterface.set_kerberos(
                        DCOMConnection.PORTMAPS[self.__target]
                        .get_rpc_transport()
                        .get_kerberos(),
                        DCOMConnection.PORTMAPS[self.__target]
                        .get_rpc_transport()
...
class IObjectExporter:
    ...
    def ResolveOxid(self, pOxid, arRequestedProtseqs):
        ...
        Oxids = b"".join(
            pack("<H", x) for x in resp["ppdsaOxidBindings"]["aStringArray"]
        )
...
class IRemoteSCMActivator:
    ...
    def RemoteGetClassObject(self, clsId, iid):
        ...
        propOutput = activationBlob["Property"][
            : activationBlob["CustomHeader"]["pSizes"][0]["Data"]
        ]
        ...
        Oxids = b"".join(
            pack("<H", x)
            for x in scmr["remoteReply"]["pdsaOxidBindings"]["aStringArray"]
        )
        strBindings = Oxids[
            : scmr["remoteReply"]["pdsaOxidBindings"]["wSecurityOffset"] * 2
        ]
        securityBindings = Oxids[
            scmr["remoteReply"]["pdsaOxidBindings"]["wSecurityOffset"] * 2 :
        ]
