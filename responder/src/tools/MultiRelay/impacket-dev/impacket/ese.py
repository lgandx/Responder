    ...
    def dump(self):
        ...
        for i in range(self.record["FirstAvailablePageTag"]):
            ...
            if (
                ...
                and self.__DBHeader["FileFormatRevision"] > 11
            ...
            print(
                "TAG %-8d offset:0x%-6x flags:0x%-4x valueSize:0x%x"
...
class ESENT_DB:
    ...
    def mountDB(self):
        ...
        LOG.debug(
            "Database Version:0x%x, Revision:0x%x"
            % (self.__DBHeader["Version"], self.__DBHeader["FileFormatRevision"])
        )
            ...
            for column in list(self.__tables[table]["Columns"].keys()):
                ...
                print(
                    ...
                    % (
                        ...
                        record["Identifier"],
                        column.decode("utf-8"),
                        ColumnTypeToName[record["ColumnType"]],
                    )
                )
    ...
    def __addLongValue(self, entry):
        ...
        lvName = entry["EntryData"][dataDefinitionHeader["VariableSizeOffset"] :][7:][
            :lvLen
        ]
            ...
            if columnRecord["Identifier"] <= dataDefinitionHeader["LastFixedSize"]:
                ...
                if columnRecord["Identifier"] in taggedItems:
                    ...
                    if itemFlag & (TAGGED_DATA_TYPE_COMPRESSED):
                        LOG.error(
                            "Unsupported tag column: %s, flag:0x%x" % (column, itemFlag)
                        )
                        record[column] = None
            ...
            if type(record[column]) is tuple:
                ...
                if record[column] is not None:
                    ...
                    try:
                        ...
                        LOG.debug(
                            "Fixing Record[%r][%d]: %r"
                            % (column, columnRecord["ColumnType"], record[column])
                        )
