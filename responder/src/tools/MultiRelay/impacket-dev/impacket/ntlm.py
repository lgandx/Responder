...
NTLMSSP_NEGOTIATE_SIGN = (
    0x00000010  # means packet is signed, if verifier is wrong it fails
)
...
class NTLMAuthChallenge(Structure):
    ...
    def getData(self):
        if (
            self["TargetInfoFields"] is not None
            and type(self["TargetInfoFields"]) is not bytes
        ):
    ...
    def fromString(self, data):
        ...
        self["TargetInfoFields"] = data[self["TargetInfoFields_offset"] :][
            : self["TargetInfoFields_len"]
        ]
        return self
...
class NTLMAuthChallengeResponse(Structure):
    ...
    def __init__(
        self, username="", password="", challenge="", lmhash="", nthash="", flags=0
    ):
        Structure.__init__(self)
        self["session_key"] = ""
        ...
        if username and (lmhash != "" or nthash != ""):
            ...
            self["ntlm"] = get_ntlmv1_response(
                nthash, challenge
            )  # This is not used for LM_KEY nor NTLM_KEY
        else:
            self["lanman"] = ""
            self["ntlm"] = ""
            if not self["host_name"]:
                self["host_name"] = "NULL".encode(
                    "utf-16le"
                )  # for NULL session there must be a hostname
    ...
    def getData(self):
        self["domain_offset"] = (
            64 + self.checkMIC(self["flags"]) + self.checkVersion(self["flags"])
        )
        self["user_offset"] = (
            64
            + self.checkMIC(self["flags"])
            + self.checkVersion(self["flags"])
...
def getNTLMSSPType1(
    workstation="", domain="", signingRequired=False, use_ntlmv2=USE_NTLMv2
):
...
def getNTLMSSPType3(
    type1, type2, user, password, domain, lmhash="", nthash="", use_ntlmv2=USE_NTLMv2
):
    ...
    ntlmChallengeResponse = NTLMAuthChallengeResponse(
        user, password, ntlmChallenge["challenge"]
    )

    clientChallenge = b(
        "".join([random.choice(string.digits + string.ascii_letters) for _ in range(8)])
    )

    serverName = ntlmChallenge["TargetInfoFields"]

    ntResponse, lmResponse, sessionBaseKey = computeResponse(
        ntlmChallenge["flags"],
    ...
    if ntlmChallenge["flags"] & NTLMSSP_NEGOTIATE_KEY_EXCH:
        ...
        encryptedRandomSessionKey = generateEncryptedSessionKey(
...
def MAC(flags, handle, signingKey, seqNum, message):
    ...
    if flags & NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY:
        if flags & NTLMSSP_NEGOTIATE_KEY_EXCH:
            ...
            messageSignature["Checksum"] = struct.unpack(
                "<q",
                handle(hmac_md5(signingKey, struct.pack("<i", seqNum) + message)[:8]),
            )[0]
            ...
            messageSignature["Version"] = 1
            messageSignature["Checksum"] = struct.unpack(
                "<q", hmac_md5(signingKey, struct.pack("<i", seqNum) + message)[:8]
            )[0]
            messageSignature["SeqNum"] = seqNum
            seqNum += 1
    else:
        messageSignature["Version"] = 1
        messageSignature["Checksum"] = struct.pack(
            "<I", binascii.crc32(message) & 0xFFFFFFFF
        )
        messageSignature["RandomPad"] = 0
        messageSignature["RandomPad"] = handle(
            struct.pack("<I", messageSignature["RandomPad"])
        )
        messageSignature["Checksum"] = struct.unpack(
            "<I", handle(messageSignature["Checksum"])
        )[0]
...
def SEAL(
    flags, signingKey, sealingKey, messageToSign, messageToEncrypt, seqNum, handle
):
...
def KXKEY(
    ...
    if flags & NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY:
        if flags & NTLMSSP_NEGOTIATE_NTLM:
            keyExchangeKey = hmac_md5(
                sessionBaseKey, serverChallenge + lmChallengeResponse[:8]
            )
        else:
            keyExchangeKey = sessionBaseKey
    elif flags & NTLMSSP_NEGOTIATE_NTLM:
        if flags & NTLMSSP_NEGOTIATE_LM_KEY:
            keyExchangeKey = __DES_block(
                LMOWFv1(password, lmhash)[:7], lmChallengeResponse[:8]
            ) + __DES_block(
                LMOWFv1(password, lmhash)[7] + b"\xbd\xbd\xbd\xbd\xbd\xbd",
...
def NTOWFv2(user, password, domain, hash=""):
    ...
    return hmac_md5(
        theHash, user.upper().encode("utf-16le") + domain.encode("utf-16le")
    )
...
def computeResponseNTLMv2(
    ...
    if TEST_CASE is False:
        av_pairs[NTLMSSP_AV_TARGET_NAME] = (
            "cifs/".encode("utf-16le") + av_pairs[NTLMSSP_AV_HOSTNAME][1]
        )
    ...
    lmChallengeResponse = (
        hmac_md5(responseKeyNT, serverChallenge + clientChallenge) + clientChallenge
    )
