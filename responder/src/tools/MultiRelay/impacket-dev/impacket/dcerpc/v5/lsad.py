...
class DCERPCSessionError(DCERPCException):
    ...
    def __str__(self):
        ...
        if key in nt_errors.ERROR_MESSAGES:
            ...
            return "LSAD SessionError: code: 0x%x - %s - %s" % (
                self.error_code,
                error_msg_short,
                error_msg_verbose,
            )
...
def hLsarEnumerateTrustedDomainsEx(
    dce, policyHandle, enumerationContext=0, preferedMaximumLength=0xFFFFFFFF
):
...
def hLsarEnumerateTrustedDomains(
    dce, policyHandle, enumerationContext=0, preferedMaximumLength=0xFFFFFFFF
):
...
def hLsarRemovePrivilegesFromAccount(
    dce, accountHandle, privileges, allPrivileges=False
):
...
def hLsarQuerySecurityObject(
    dce, policyHandle, securityInformation=OWNER_SECURITY_INFORMATION
):
