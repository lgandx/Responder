...
class HTTPClientSecurityProvider:
    ...
    def get_auth_headers_basic(self, http_obj, method, path, headers):
        if (
            ...
            raise Exception(
                "Basic authentication in HTTP connection used, "
        ...
        auth_line_http = "Basic %s" % base64.b64encode(
            auth_line.encode("UTF-8")
        ).decode("ascii")
    ...
    def send_ntlm_type1(self, http_obj, method, path, headers, negotiateMessage):
        ...
        if res.status != 401:
            raise Exception(
                ...
                "Authentication does not seem required for url %s" % (res.status, path)
            )

        if res.getheader("WWW-Authenticate") is None:
            raise Exception(
                "No authentication requested by " "the server for url %s" % path
            )
    ...
    def get_auth_headers_auto(self, http_obj, method, path, headers):
        if self.__aesKey != "" or self.__TGT != None or self.__TGS != None:
            raise Exception(
                "NTLM authentication in HTTP connection used, " "cannot use Kerberos."
            )

        auth = ntlm.getNTLMSSPType1(domain=self.__domain)
        serverChallenge = self.send_ntlm_type1(
            http_obj, method, path, headers, auth.getData()
        )[0]
        ...
        if serverChallenge is not None:
            ...
            auth_line_http = "NTLM %s" % base64.b64encode(type3.getData()).decode(
                "ascii"
            )
        else:
            if self.__auth_type == AUTH_AUTO and AUTH_BASIC in self.__auth_types:
                ...
                raise Exception(
                    "No supported auth offered by URL: %s" % self.__auth_types
                )
