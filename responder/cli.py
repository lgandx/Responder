...
from responder.src import settings, servers, tools

banner()


parser = optparse.OptionParser(
    usage="python %prog -I eth0 -w -d\nor:\npython %prog -I eth0 -wd",
...
parser.add_option(
    ...
    "--externalip6",
    action="store",
    help="Poison all requests with another IPv6 address than Responder's one.",
    dest="ExternalIP6",
    metavar="2002:c0a8:f7:1:3ba8:aceb:b1a9:81ed",
    default=None,
)
parser.add_option(
    "-e",
...
parser.add_option(
    ...
    "--DHCP",
    action="store_true",
    help="Enable answers for DHCP broadcast requests. This option will inject a WPAD server in the DHCP response. Default: False",
    dest="DHCP_On_Off",
    default=False,
)
parser.add_option(
    ...
    "--DHCP-DNS",
    action="store_true",
    help="This option will inject a DNS server in the DHCP response, otherwise a WPAD server will be added. Default: False",
    dest="DHCP_DNS",
    default=False,
)
...
parser.add_option(
    ...
    "--ProxyAuth",
    action="store_true",
    help="Force NTLM (transparently)/Basic (prompt) authentication for the proxy. WPAD doesn't need to be ON. This option is highly effective. Default: False",
...
parser.add_option(
    ...
    "--quiet",
    action="store_true",
    help="Tell Responder to be quiet, disables a bunch of printing from the poisoners. Default: False",
    dest="Quiet",
    default=False,
)
...
parser.add_option(
    "--disable-ess",
    action="store_true",
    help="Force ESS downgrade. Default: False",
    dest="NOESS_On_Off",
    default=False,
)
parser.add_option(
    "-v", "--verbose", action="store_true", help="Increase verbosity.", dest="Verbose"
)
parser.add_option(
    "-t",
    "--ttl",
    action="store",
    help="Change the default Windows TTL for poisoned answers. Value in hex (30 seconds = 1e). use '-t random' for random TTL",
    dest="TTL",
    metavar="1e",
    default=None,
)
parser.add_option(
    ...
    "--AnswerName",
    action="store",
    help="Specifies the canonical name returned by the LLMNR poisoner in its Answer section. By default, the answer's canonical name is the same as the query. Changing this value is mainly useful when attempting to perform Kerberos relaying over HTTP.",
    dest="AnswerName",
    default=None,
)
parser.add_option(
    ...
    "--ErrorCode",
    action="store_true",
    help="Changes the error code returned by the SMB server to STATUS_LOGON_FAILURE. By default, the status is STATUS_ACCESS_DENIED. Changing this value permits to obtain WebDAV authentications from the poisoned machines where the WebClient service is running.",
    dest="ErrorCode",
    default=False,
)
...
def serve_thread_udp_broadcast(host, port, handler):
    try:
        ...
        print(
            color("[!] ", 1, 1)
            + "Error starting UDP server on port "
            + str(port)
...
def serve_thread_SSL(host, port, handler):
    try:
        # Use resolved SSL certificate paths from settings
        cert = settings.Config.SSLCert
        key = settings.Config.SSLKey
        ...
        print(
            color("[!] ", 1, 1)
            + "Error starting SSL server on port "
            + str(port)
            + ", check permissions or other servers running."
        )
...
def main():
    try:
        ...
        if settings.Config.LLMNR_On_Off:
            from responder.src.poisoners.LLMNR import LLMNR
        ...
        if settings.Config.NBTNS_On_Off:
            from responder.src.poisoners.NBTNS import NBTNS
        ...
        if settings.Config.MDNS_On_Off:
            from responder.src.poisoners.MDNS import MDNS
        ...
        if settings.Config.HTTP_On_Off:
            from responder.src.servers.HTTP import HTTP
        ...
        if settings.Config.WinRM_On_Off:
            from responder.src.servers.WinRM import WinRM
        ...
        if settings.Config.RDP_On_Off:
            from responder.src.servers.RDP import RDP
        ...
        if settings.Config.DCERPC_On_Off:
            from responder.src.servers.RPC import RPCMap, RPCMapper
        ...
        if settings.Config.WPAD_On_Off:
            from responder.src.servers.HTTP_Proxy import HTTP_Proxy
        ...
        if settings.Config.ProxyAuth_On_Off:
            from responder.src.servers.Proxy_Auth import Proxy_Auth
        ...
        if settings.Config.SMB_On_Off:
            if settings.Config.LM_On_Off:
                from responder.src.servers.SMB import SMB1LM
                ...
                from responder.src.servers.SMB import SMB1
        ...
        if settings.Config.QUIC_On_Off:
            from responder.src.servers.QUIC import start_quic_server
            ...
            threads.append(
                Thread(
                    target=lambda: asyncio.run(
                        start_quic_server(settings.Config.Bind_To, cert, key)
                    )
                )
            )

        if settings.Config.Krb_On_Off:
            from responder.src.servers.Kerberos import KerbTCP, KerbUDP
        ...
        if settings.Config.SQL_On_Off:
            from responder.src.servers.MSSQL import MSSQL, MSSQLBrowser
        ...
        if settings.Config.FTP_On_Off:
            from responder.src.servers.FTP import FTP
        ...
        if settings.Config.POP_On_Off:
            from responder.src.servers.POP3 import POP3
        ...
        if settings.Config.LDAP_On_Off:
            from responder.src.servers.LDAP import LDAP, CLDAP
        ...
        if settings.Config.MQTT_On_Off:
            from responder.src.servers.MQTT import MQTT
        ...
        if settings.Config.SMTP_On_Off:
            from responder.src.servers.SMTP import ESMTP
        ...
        if settings.Config.IMAP_On_Off:
            from responder.src.servers.IMAP import IMAP
        ...
        if settings.Config.DNS_On_Off:
            from responder.src.servers.DNS import DNS, DNSTCP
        ...
        if settings.Config.SNMP_On_Off:
            from responder.src.servers.SNMP import SNMP
        ...
        if settings.Config.DHCP_On_Off:
            from responder.src.poisoners.DHCP import DHCP
