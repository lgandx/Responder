...
class NDR(object):
    ...
    def __init__(self, data=None, isNDR64=False):
        ...
        for fieldName, fieldTypeOrClass in (
            self.commonHdr + self.structure + self.referent
        ):
        ...
        if newSyntax == NDR64Syntax:
            if self._isNDR64 is False:
                ...
                for fieldName, fieldTypeOrClass in (
                    ...
                    if isinstance(self.fields[fieldName], NDR):
                        if (
                            fieldTypeOrClass != self.fields[fieldName].__class__
                            and isinstance(self.fields[fieldName], NDRPOINTERNULL)
                            is False
                        ):
                            backupData = self[fieldName]
                            self.fields[fieldName] = fieldTypeOrClass(
                                isNDR64=self._isNDR64
                            )
    ...
    def __setitem__(self, key, value):
        if isinstance(value, NDRPOINTERNULL):
            ...
            if self.fields[key].__class__.__name__ == value.__class__.__name__:
                ...
                if (
                    self.fields[key]["Data"].__class__.__name__
                    == value.__class__.__name__
                ):
                    self.fields[key]["Data"] = value
                else:
                    LOG.error(
                        "Can't setitem with class specified, should be %s"
                        % self.fields[key]["Data"].__class__.__name__
                    )
            else:
                LOG.error(
                    "Can't setitem with class specified, should be %s"
                    % self.fields[key].__class__.__name__
                )
    ...
    def getData(self, soFar=0):
        ...
        for fieldName, fieldTypeOrClass in self.commonHdr + self.structure:
            try:
                ...
                LOG.error(str(e))
                LOG.error(
                    "Error packing field '%s | %s' in %s"
                    % (fieldName, fieldTypeOrClass, self.__class__)
                )
                raise
    ...
    def fromString(self, data, offset=0):
        ...
        for fieldName, fieldTypeOrClass in self.commonHdr + self.structure:
            try:
                ...
                LOG.error(str(e))
                LOG.error(
                    "Error unpacking field '%s | %s | %r'"
                    % (fieldName, fieldTypeOrClass, data[offset : offset + 256])
                )
                raise
...
class NDRCONSTRUCTEDTYPE(NDR):
    ...
    def getDataReferent(self, soFar=0):
        ...
        for fieldName, fieldTypeOrClass in self.referent:
            try:
                if isinstance(
                    self.fields[fieldName], NDRUniConformantArray
                ) or isinstance(self.fields[fieldName], NDRUniConformantVaryingArray):
                    ...
                    data = (
                        arrayPadding
                        + pack(arrayPackStr, self.getArrayMaximumSize(fieldName))
                        + data
                    )
                else:
                    pad = self.calculatePad(fieldTypeOrClass, soFar)
    ...
    def getArrayMaximumSize(self, fieldName):
        if (
            self.fields[fieldName].fields["MaximumCount"] is not None
            and self.fields[fieldName].fields["MaximumCount"] > 0
        ):
        ...
        for fieldName, fieldTypeOrClass in self.referent:
            try:
                if isinstance(
                    ...
                    # Get the array size
                    arraySize, advanceStream = self.getArraySize(
                ...
                if isinstance(self.fields[fieldName], NDRCONSTRUCTEDTYPE):
                    size += self.fields[fieldName].fromStringReferents(
                        data, offset + size
                    )
                    size += self.fields[fieldName].fromStringReferent(
                        data, offset + size
                    )
                offset += size
            except Exception as e:
...
class NDRArray(NDRCONSTRUCTEDTYPE):
    ...
    def pack(self, fieldName, fieldTypeOrClass, soFar=0):
        ...
        if len(two) == 2:
            ...
            if isinstance(self, NDRUniConformantArray) or isinstance(
                self, NDRUniConformantVaryingArray
            ):
    ...
    def unpack(self, fieldName, fieldTypeOrClass, data, offset=0):
        ...
        if len(two) == 2:
            ...
            if dataClassOrCode is not None and isinstance(
                dataClassOrCode(), NDRCONSTRUCTEDTYPE
            ):
            ...
            return NDRCONSTRUCTEDTYPE.unpack(
                self, fieldName, fieldTypeOrClass, data, offset
            )
...
class NDRSTRUCT(NDRCONSTRUCTEDTYPE):
    def getData(self, soFar=0):
        ...
        if isinstance(self.fields[lastItem], NDRUniConformantArray) or isinstance(
            self.fields[lastItem], NDRUniConformantVaryingArray
        ):
            # So we have an array, first item in the structure must be the array size, although we
            # will need to build it later.
            if self._isNDR64:
...
class NDRUNION(NDRCONSTRUCTEDTYPE):
    commonHdr = (("tag", NDRUSHORT),)
    commonHdr64 = (("tag", NDRULONG),)
        ...
        for fieldName, fieldTypeOrClass in (
            ...
            if self.isNDR(fieldTypeOrClass):
                if self.isPointer(fieldTypeOrClass):
                    self.fields[fieldName] = fieldTypeOrClass(
                        isNDR64=self._isNDR64, topLevel=topLevel
                    )
            ...
            if value in self.union:
                ...
                if "default" in self.union:
                    if self.union["default"] is None:
                        ...
                        # Init again the structure
                        self.__init__(
                            None, isNDR64=self._isNDR64, topLevel=self.topLevel
                        )
...
class NDRPOINTER(NDRSTRUCT):
    align = 4
    align64 = 8
    commonHdr = (("ReferentID", "<L=0xff"),)
    commonHdr64 = (("ReferentID", "<Q=0xff"),)
...
class PNDRUniConformantArray(NDRPOINTER):
    referent = (("Data", NDRUniConformantArray),)
    ...
    def __init__(self, data=None, isNDR64=False):
        ...
        for fieldName, fieldTypeOrClass in (
            ...
            if self.isNDR(fieldTypeOrClass):
                if self.isPointer(fieldTypeOrClass):
                    self.fields[fieldName] = fieldTypeOrClass(
                        isNDR64=self._isNDR64, topLevel=True
                    )
                elif self.isUnion(fieldTypeOrClass):
                    self.fields[fieldName] = fieldTypeOrClass(
                        isNDR64=self._isNDR64, topLevel=True
