...
class DCERPCSessionError(DCERPCException):
    ...
    def __str__(self):
        ...
        if key in system_errors.ERROR_MESSAGES:
            ...
            return "NRPC SessionError: code: 0x%x - %s - %s" % (
                self.error_code,
                error_msg_short,
                error_msg_verbose,
            )
...
def ComputeSessionKeyStrongKey(
    sharedSecret, clientChallenge, serverChallenge, sharedSecretHash=None
):
    # added the ability to receive hashes already
...
def SIGN(data, confounder, sequenceNum, key, aes=False):
    if aes is False:
        ...
        signature["Checksum"] = ComputeNetlogonSignatureMD5(
            signature, data, confounder, key
        )
        signature["SequenceNumber"] = encryptSequenceNumberRC4(
            deriveSequenceNumber(sequenceNum), signature["Checksum"], key
        )
        return signature
    else:
        signature = NL_AUTH_SIGNATURE()
        ...
        signature["Checksum"] = ComputeNetlogonSignatureAES(
            signature, data, confounder, key
        )
        signature["SequenceNumber"] = encryptSequenceNumberAES(
            deriveSequenceNumber(sequenceNum), signature["Checksum"], key
        )
        return signature
...
def UNSEAL(data, auth_data, key, aes=False):
    ...
    if aes is False:
        sequenceNum = decryptSequenceNumberRC4(
            auth_data["SequenceNumber"], auth_data["Checksum"], key
        )
        hm = hmac.new(XorKey, digestmod=hashlib.md5)
        hm.update(b"\x00" * 4)
        hm2 = hmac.new(hm.digest(), digestmod=hashlib.md5)
        ...
        sequenceNum = decryptSequenceNumberAES(
            auth_data["SequenceNumber"], auth_data["Checksum"], key
        )
        IV = sequenceNum + sequenceNum
        cipher = AES.new(XorKey, AES.MODE_CFB, IV)
...
OPNUMS = {
    ...
    35: (
        ...
        NetrLogonGetTimeServiceParentDomainResponse,
    ),
...
def hNetrServerTrustPasswordsGet(
