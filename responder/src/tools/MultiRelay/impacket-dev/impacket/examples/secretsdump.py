...
class SAMHashes(OfflineRegistry):
    ...
    def getHBootKey(self):
        ...
        if domainData["Key0"][0:1] == b"\x01":
            ...
            rc4 = ARC4.new(rc4Key)
            self.__hashedBootKey = rc4.encrypt(
                samKeyData["Key"] + samKeyData["CheckSum"]
            )

            # Verify key with checksum
            checkSum = self.MD5(
                self.__hashedBootKey[:16] + DIGITS + self.__hashedBootKey[:16] + QWERTY
            )
            ...
            self.__hashedBootKey = self.__cryptoCommon.decryptAES(
                self.__bootKey,
    ...
    def __decryptHash(self, rid, cryptedHash, constant, newStyle=False):
        ...
        if newStyle is False:
            ...
            key = self.__cryptoCommon.decryptAES(
                self.__hashedBootKey[:0x10], cryptedHash["Hash"], cryptedHash["Salt"]
            )[:16]
    ...
    def dump(self):
        ...
        for rid in rids:
            ...
            userName = V[
                ...
                + userAccount["NameLength"]
            ].decode("utf-16le")

            if userAccount["NTHashLength"] == 0:
                logging.error(
                    "SAM hashes extraction for user %s failed. The account doesn't have hash information."
                    % userName
                )
                continue
            ...
            if V[userAccount["NTHashOffset"] :][2:3] == b"\x01":
                ...
                if userAccount["LMHashLength"] == 20:
                    encLMHash = SAM_HASH(
                        V[userAccount["LMHashOffset"] :][: userAccount["LMHashLength"]]
                    )
                if userAccount["NTHashLength"] == 20:
                    encNTHash = SAM_HASH(
                        V[userAccount["NTHashOffset"] :][: userAccount["NTHashLength"]]
                    )
            else:
                # New Style hashes
                newStyle = True
...
class LSASecrets(OfflineRegistry):
    ...
    def __decryptLSA(self, value):
        if self.__vistaStyle is True:
            ...
            plainText = self.__cryptoCommon.decryptAES(
                tmpKey, record["EncryptedData"][32:]
            )
    ...
    def __getNLKMSecret(self):
        ...
        if self.__vistaStyle is True:
            ...
            self.__NKLMKey = self.__cryptoCommon.decryptAES(
                tmpKey, record["EncryptedData"][32:]
            )
        else:
    ...
    def dumpCachedHashes(self):
        ...
        for value in values:
            ...
            record = NL_RECORD(
                self.getValue(ntpath.join("\\Cache", value.decode("utf-8")))[1]
            )
            if record["IV"] != 16 * b"\x00":
                ...
                if record["Flags"] & 1 == 1:
                    ...
                    if self.__vistaStyle is True:
                        plainText = self.__cryptoCommon.decryptAES(
                            self.__NKLMKey[16:32], record["EncryptedData"], record["IV"]
                        )
                    else:
                        plainText = self.__decryptHash(
                            self.__NKLMKey, record["EncryptedData"], record["IV"]
                        )
                        pass
                else:
                ...
                domainLong = plainText[
                    : self.__pad(record["DnsDomainNameLength"])
                ].decode("utf-16le")

                if self.__vistaStyle is True:
                    answer = "%s/%s:$DCC2$%s#%s#%s" % (
                        domainLong,
                        userName,
                        iterationCount,
                        userName,
    ...
    def __printSecret(self, name, secretItem):
        ...
        if upperName.startswith("_SC_"):
            ...
            secret = "dpapi_machinekey:0x{0}\ndpapi_userkey:0x{1}".format(
                hexlify(dpapi["MachineKey"]).decode("latin-1"),
            ...
            extrasecret = "%s:plain_password_hex:%s" % (
                printname,
                hexlify(secretItem).decode("utf-8"),
            )
    ...
    def __printMachineKerberos(self, rawsecret, machinename):
        ...
        if hasattr(self.__remoteOps, "getMachineKerberosSalt"):
            ...
            if salt == b"":
                ...
                rawsecret = rawsecret.decode("utf-16-le", "replace").encode(
                    "utf-8", "replace"
                )
                for etype in allciphers:
                    ...
                    self.__secretItems.append(secret)
                    self.__perSecretCallback(
                        LSASecrets.SECRET_TYPE.LSA_KERBEROS, secret
                    )
                return True
        else:
            return False
    ...
    def dumpSecrets(self):
        ...
        for key in keys:
            ...
            for valueType in valueTypeList:
                value = self.getValue(
                    "\\Policy\\Secrets\\{}\\{}\\default".format(key, valueType)
                )
                if value is not None and value[1] != 0:
                    if self.__vistaStyle is True:
                        record = LSA_SECRET(value[1])
                        tmpKey = self.__sha256(
                            self.__LSAKey, record["EncryptedData"][:32]
                        )
                        plainText = self.__cryptoCommon.decryptAES(
                            tmpKey, record["EncryptedData"][32:]
                        )
...
class ResumeSessionMgrInFile(object):
    ...
    def beginTransaction(self):
        if not self.__resumeFileName:
            self.__resumeFileName = "sessionresume_%s" % "".join(
        ...
        if not self.__resumeFile:
            try:
                ...
                raise Exception(
                    'Cannot create "%s" resume session file: %s'
                    % (self.__resumeFileName, str(e))
                )
...
class NTDSHashes:
        ...
        if peklist is not None:
            ...
            if encryptedPekList["Header"][:4] == b"\x02\x00\x00\x00":
                ...
                for i in range(len(decryptedPekList["DecryptedPek"]) // PEKLen):
                    ...
                    LOG.info(
                        "PEK # %d found and decrypted: %s",
                        i,
                        hexlify(pek["Key"]).decode("utf-8"),
                    )
    ...
    def __decryptSupplementalInfo(
        self, record, prefixTable=None, keysFile=None, clearTextFile=None
    ):
        ...
        if self.__useVSSMethod is True:
            if record[self.NAME_TO_INTERNAL["supplementalCredentials"]] is not None:
                if (
                    len(
                        unhexlify(
                            record[self.NAME_TO_INTERNAL["supplementalCredentials"]]
                        )
                    )
                    > 24
                ):
                    if record[self.NAME_TO_INTERNAL["userPrincipalName"]] is not None:
                        domain = record[
                            self.NAME_TO_INTERNAL["userPrincipalName"]
                        ].split("@")[-1]
                        userName = "%s\\%s" % (
                            domain,
                            record[self.NAME_TO_INTERNAL["sAMAccountName"]],
                        )
                    else:
                        userName = (
                            "%s" % record[self.NAME_TO_INTERNAL["sAMAccountName"]]
                        )
            ...
            for attr in record["pmsgOut"][replyVersion]["pObjects"]["Entinf"][
                ...
                if attId == LOOKUP_TABLE["userPrincipalName"]:
                    if attr["AttrVal"]["valCount"] > 0:
                        try:
                            domain = (
                                b"".join(attr["AttrVal"]["pAVal"][0]["pVal"])
                                .decode("utf-16le")
                                .split("@")[-1]
                            )
                        except:
                    ...
                    if attr["AttrVal"]["valCount"] > 0:
                        try:
                            ...
                            LOG.error(
                                ...
                                % record["pmsgOut"][replyVersion]["pNC"]["StringName"][
                                    :-1
                                ]
                            )
                            userName = "unknown"
        ...
        if haveInfo is True:
            ...
            for propertyCount in range(userProperties["PropertyCount"]):
                ...
                if (
                    userProperty["PropertyName"].decode("utf-16le")
                    ...
                    for credential in range(kerbStoredCredentialNew["CredentialCount"]):
                        ...
                        keyValue = propertyValueBuffer[keyDataNew["KeyOffset"] :][
                            : keyDataNew["KeyLength"]
                        ]
    ...
    def __decryptHash(self, record, prefixTable=None, outputFile=None):
        ...
        if self.__useVSSMethod is True:
            LOG.debug(
                "Decrypting hash for user: %s" % record[self.NAME_TO_INTERNAL["name"]]
            )
            ...
            if record[self.NAME_TO_INTERNAL["dBCSPwd"]] is not None:
                ...
                if encryptedLMHash["Header"][:4] == b"\x13\x00\x00\x00":
                    ...
                    tmpLMHash = self.__cryptoCommon.decryptAES(
                        self.__PEK[int(pekIndex[8:10])],
            ...
            if record[self.NAME_TO_INTERNAL["unicodePwd"]] is not None:
                ...
                if encryptedNTHash["Header"][:4] == b"\x13\x00\x00\x00":
                    ...
                    tmpNTHash = self.__cryptoCommon.decryptAES(
                        self.__PEK[int(pekIndex[8:10])],
            ...
            if self.__printUserStatus is True:
                ...
                if record[self.NAME_TO_INTERNAL["userAccountControl"]] is not None:
                    if (
                        "{0:08b}".format(
                            record[self.NAME_TO_INTERNAL["userAccountControl"]]
                        )[-2:-1]
                        == "1"
                    ):
            ...
            if self.__history:
                ...
                if record[self.NAME_TO_INTERNAL["ntPwdHistory"]] is not None:
                    ...
                    if encryptedNTHistory["Header"][:4] == b"\x13\x00\x00\x00":
                        ...
                        tmpNTHistory = self.__cryptoCommon.decryptAES(
                            self.__PEK[int(pekIndex[8:10])],
            ...
            rid = unpack(
                "<L",
                record["pmsgOut"][replyVersion]["pObjects"]["Entinf"]["pName"]["Sid"][
                    -4:
                ],
            )[0]
                ...
                if attId == LOOKUP_TABLE["dBCSPwd"]:
                    ...
                    if attr["AttrVal"]["valCount"] > 0:
                        try:
                            pwdLastSet = self.__fileTimeToDateTime(
                                unpack(
                                    "<Q", b"".join(attr["AttrVal"]["pAVal"][0]["pVal"])
                                )[0]
                            )
                        except:
                            LOG.error(
                                "Cannot get pwdLastSet for %s"
                                % record["pmsgOut"][replyVersion]["pNC"]["StringName"][
                                    :-1
                                ]
                            )
                    ...
                    and attId == LOOKUP_TABLE["userAccountControl"]
                ):
                    if attr["AttrVal"]["valCount"] > 0:
                        if (
                            unpack("<L", b"".join(attr["AttrVal"]["pAVal"][0]["pVal"]))[
                                0
                            ]
                        ) & samr.UF_ACCOUNTDISABLE:
                            userAccountStatus = "Disabled"
                        else:
                            userAccountStatus = "Enabled"
                    else:
            ...
            if self.__NTDS is None:
                ...
                try:
                    if self.__remoteOps is not None:
                        try:
                            self.__remoteOps.connectSamr(
                                self.__remoteOps.getMachineNameAndDomain()[1]
                            )
                        except:
                            if (
                                os.getenv("KRB5CCNAME") is not None
                                and self.__justUser is not None
                            ):
        ...
        try:
            ...
            if self.__outputFileName is not None:
                ...
                if self.__justNTLM is False:
                    keysOutputFile = openFile(
                        self.__outputFileName + ".ntds.kerberos", mode
                    )
                    clearTextOutputFile = openFile(
                        self.__outputFileName + ".ntds.cleartext", mode
                    )
            ...
            if self.__useVSSMethod:
                ...
                if self.__PEK is not None:
                        ...
                        try:
                            ...
                            if self.__justNTLM is False:
                                self.__decryptSupplementalInfo(
                                    record, None, keysOutputFile, clearTextOutputFile
                                )
                        except Exception as e:
                            LOG.debug("Exception", exc_info=True)
                            try:
                    ...
                    while True:
                        ...
                        try:
                            if (
                                record[self.NAME_TO_INTERNAL["sAMAccountType"]]
                                in self.ACCOUNT_TYPES
                            ):
                                self.__decryptHash(record, outputFile=hashesOutputFile)
                                if self.__justNTLM is False:
                                    self.__decryptSupplementalInfo(
                ...
                if self.__justUser is not None:
                    ...
                    if (
                        self.__justUser.find("\\") >= 0
                        or self.__justUser.find("/") >= 0
                    ):
                    ...
                    if crackedName["pmsgOut"]["V1"]["pResult"]["cItems"] == 1:
                        if (
                            crackedName["pmsgOut"]["V1"]["pResult"]["rItems"][0][
                                "status"
                            ]
                            != 0
                        ):
                            raise Exception(
                                "%s: %s"
                                % system_errors.ERROR_MESSAGES[
                                    0x2114
                                    + crackedName["pmsgOut"]["V1"]["pResult"]["rItems"][
                                        0
                                    ]["status"]
                                ]
                            )
                        ...
                        LOG.warning(
                            "DRSCrackNames returned %d items for user %s, skipping"
                            % (
                                crackedName["pmsgOut"]["V1"]["pResult"]["cItems"],
                                self.__justUser,
                            )
                        )
                    try:
                        ...
                        for user in resp["Buffer"]["Buffer"]:
                            ...
                            if resumeSid is not None:
                                ...
                                if resumeSid == userSid:
                                    ...
                                    LOG.debug(
                                        "resumeSid %s reached! processing users from now on"
                                        % userSid
                                    )
                                    resumeSid = None
                                else:
                                    LOG.debug(
                                        "Skipping SID %s since it was processed already"
                                        % userSid
                                    )
                                continue
                            ...
                            if crackedName["pmsgOut"]["V1"]["pResult"]["cItems"] == 1:
                                ...
                                replyVersion = "V%d" % userRecord["pdwOutVersion"]
                                if (
                                    userRecord["pmsgOut"][replyVersion]["cNumObjects"]
                                    == 0
                                ):
                                    raise Exception(
                            ...
                            try:
                                self.__decryptHash(
                                    userRecord,
                                    userRecord["pmsgOut"][replyVersion][
                                        "PrefixTableSrc"
                                    ]["pPrefixEntry"],
                                    hashesOutputFile,
                                )
            ...
            if len(self.__kerberosKeys) > 0:
                ...
                for itemKey in list(self.__kerberosKeys.keys()):
                    self.__perSecretCallback(
                        NTDSHashes.SECRET_TYPE.NTDS_KERBEROS, itemKey
                    )
            ...
            if len(self.__clearTextPwds) > 0:
                ...
                for itemKey in list(self.__clearTextPwds.keys()):
                    self.__perSecretCallback(
                        NTDSHashes.SECRET_TYPE.NTDS_CLEARTEXT, itemKey
                    )
        finally:
            # Resources cleanup
