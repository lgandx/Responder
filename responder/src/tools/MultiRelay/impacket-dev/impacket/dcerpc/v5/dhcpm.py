...
class DCERPCSessionError(DCERPCException):
    ...
    def __str__(self):
        ...
        if key in system_errors.ERROR_MESSAGES:
            ...
            return "DHCPM SessionError: code: 0x%x - %s - %s" % (
                self.error_code,
                error_msg_short,
                error_msg_verbose,
            )
...
def hDhcpGetOptionValue(
    dce, optionID, scopetype=DHCP_OPTION_SCOPE_TYPE.DhcpDefaultOptions, options=NULL
):
    ...
    if (
        scopetype != DHCP_OPTION_SCOPE_TYPE.DhcpDefaultOptions
        and scopetype != DHCP_OPTION_SCOPE_TYPE.DhcpGlobalOptions
    ):
...
def hDhcpEnumOptionValuesV5(
    dce,
    flags=DHCP_FLAGS_OPTION_DEFAULT,
    classname=NULL,
    vendorname=NULL,
    scopetype=DHCP_OPTION_SCOPE_TYPE.DhcpDefaultOptions,
    options=NULL,
...
def hDhcpGetAllOptionValues(
    dce, scopetype=DHCP_OPTION_SCOPE_TYPE.DhcpDefaultOptions, options=NULL
):
...
def hDhcpEnumSubnetElementsV5(
    dce,
    subnet_address,
    element_type=DHCP_SUBNET_ELEMENT_TYPE.DhcpIpRanges,
    preferredMaximum=0xFFFFFFFF,
):
