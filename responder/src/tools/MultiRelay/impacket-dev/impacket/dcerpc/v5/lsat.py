...
class DCERPCSessionError(DCERPCException):
    ...
    def __str__(self):
        ...
        if key in nt_errors.ERROR_MESSAGES:
            ...
            return "LSAT SessionError: code: 0x%x - %s - %s" % (
                self.error_code,
                error_msg_short,
                error_msg_verbose,
            )
...
def hLsarLookupNames4(
    dce,
    names,
    lookupLevel=LSAP_LOOKUP_LEVEL.LsapLookupWksta,
...
def hLsarLookupNames(
    dce, policyHandle, names, lookupLevel=LSAP_LOOKUP_LEVEL.LsapLookupWksta
):
...
def hLsarLookupSids(
    dce, policyHandle, sids, lookupLevel=LSAP_LOOKUP_LEVEL.LsapLookupWksta
):
