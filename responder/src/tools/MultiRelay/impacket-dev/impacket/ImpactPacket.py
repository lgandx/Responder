...
class Header(PacketBuffer, ProtocolLayer):
    ...
    def get_header_size(self):
        ...
        raise RuntimeError(
            "Method %s.get_header_size must be overridden." % self.__class__
        )
    ...
    def add_option(self, option):
        ...
        if sum > 40:
            raise ImpactPacketException(
                "Options overflowed in IP packet with length: %d" % sum
            )
...
class IPOption(PacketBuffer):
    ...
    def append_ip(self, ip):
        op = self.get_code()
        if not (
            op == IPOption.IPOPT_RR
            or op == IPOption.IPOPT_LSRR
            or op == IPOption.IPOPT_SSRR
            or op == IPOption.IPOPT_TS
        ):
            raise ImpactPacketException(
...
class ICMP(Header):
    ...
    def get_header_size(self):
        anamolies = {
            ICMP.ICMP_TSTAMP: 20,
            ICMP.ICMP_TSTAMPREPLY: 20,
            ICMP.ICMP_MASKREQ: 12,
            ICMP.ICMP_MASKREPLY: 12,
        }
    ...
    def get_code_name(self, aType, aCode):
        tmp_code = {
            3: [
                ...
                "UNREACH SRCFAIL",
                "UNREACH NET UNKNOWN",
                "UNREACH HOST UNKNOWN",
                "UNREACH ISOLATED",
                "UNREACH NET PROHIB",
                "UNREACH HOST PROHIB",
                "UNREACH TOSNET",
                "UNREACH TOSHOST",
                "UNREACH FILTER PROHIB",
                "UNREACH HOST PRECEDENCE",
                "UNREACH PRECEDENCE CUTOFF",
        ...
        tmp_code[40] = [
            None,
            "PHOTURIS UNKNOWN INDEX",
            "PHOTURIS AUTH FAILED",
            "PHOTURIS DECRYPT FAILED",
        ]
...
class IGMP(Header):
    ...
    def get_type_name(self, aType):
        tmp_dict = {
            ...
            0x12: "v1 HOST MEMBERSHIP REPORT ",
            0x13: "IGMP DVMRP ",
            0x14: " PIM ",
            0x16: "v2 HOST MEMBERSHIP REPORT ",
            0x17: "HOST LEAVE MESSAGE ",
            0x1E: "MTRACE REPLY ",
            0x1F: "MTRACE QUERY ",
        }
    ...
    def __str__(self):
        ...
        tmp_str += "Group: " + socket.inet_ntoa(
            struct.pack("!L", self.get_igmp_group())
        )
        if self.child():
            tmp_str += "\n" + str(self.child())
        return tmp_str
...
class ARP(Header):
    ...
    def get_ar_spa(self):
        ...
        return self.get_bytes().tolist()[
            8 + self.get_ar_hln() : 8 + self.get_ar_hln() + tmp_size
        ]
