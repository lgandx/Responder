...
class DCERPCSessionError(DCERPCException):
    ...
    def __str__(self):
        ...
        if key in system_errors.ERROR_MESSAGES:
            ...
            return "SRVS SessionError: code: 0x%x - %s - %s" % (
                self.error_code,
                error_msg_short,
                error_msg_verbose,
            )
    ...
    def __setitem__(self, key, value):
        if key == "Data":
            try:
                ...
                self.fields[key] = value.decode(sys.getfilesystemencoding()).encode(
                    "utf-16le"
                )
...
# HELPER FUNCTIONS
################################################################################
def hNetrConnectionEnum(
    dce, qualifier, level, resumeHandle=0, preferedMaximumLength=0xFFFFFFFF
):
...
def hNetrFileEnum(
    dce, basePath, userName, level, resumeHandle=0, preferedMaximumLength=0xFFFFFFFF
):
...
def hNetrSessionEnum(
    dce, clientName, userName, level, resumeHandle=0, preferedMaximumLength=0xFFFFFFFF
):
...
def hNetrServerTransportEnum(
    dce, level, resumeHandle=0, preferedMaximumLength=0xFFFFFFFF
):
...
def hNetrpSetFileSecurity(
    dce, shareName, lpFileName, securityInformation, securityDescriptor
):
