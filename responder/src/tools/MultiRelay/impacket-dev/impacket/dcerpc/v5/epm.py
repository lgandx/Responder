...
class DCERPCSessionError(DCERPCException):
    ...
    def __str__(self):
        ...
        if key in self.error_messages:
            ...
            return "EPM SessionError: code: 0x%x - %s " % (
                self.error_code,
                error_msg_short,
            )
        else:
...
def hept_lookup(
    destHost,
    inquiry_type=RPC_C_EP_ALL_ELTS,
    objectUUID=NULL,
    ifId=NULL,
...
def hept_map(
    destHost,
    remoteIf,
    dataRepresentation=uuidtup_to_bin(("8a885d04-1ceb-11c9-9fe8-08002b104860", "2.0")),
    protocol="ncacn_np",
    dce=None,
):
    ...
    tower["Floors"] = (
        interface.getData() + dataRep.getData() + protId.getData() + transportData
    )
...
def PrintStringBinding(floors):
    ...
    for floor in floors[3:]:
        if floor["ProtocolData"] == b"\x07":
            ...
            tmp_address = "ncacn_np:%%s[%s]" % floor["RelatedData"][
                : len(floor["RelatedData"]) - 1
            ].decode("utf-8")
        elif floor["ProtocolData"] == b"\x10":
            return "ncalrpc:[%s]" % floor["RelatedData"][
                : len(floor["RelatedData"]) - 1
            ].decode("utf-8")
        elif floor["ProtocolData"] == b"\x01" or floor["ProtocolData"] == b"\x11":
            if tmp_address != "":
                return tmp_address % floor["RelatedData"][
                    : len(floor["RelatedData"]) - 1
                ].decode("utf-8")
            else:
