...
class DCERPCSessionError(DCERPCException):
    ...
    def __str__(self):
        ...
        if key in nt_errors.ERROR_MESSAGES:
            ...
            return "SAMR SessionError: code: 0x%x - %s - %s" % (
                self.error_code,
                error_msg_short,
                error_msg_verbose,
            )
...
def hSamrConnect5(
    dce, serverName="\x00", desiredAccess=MAXIMUM_ALLOWED, inVersion=1, revision=3
):
...
def hSamrConnect4(
    dce, serverName="\x00", desiredAccess=MAXIMUM_ALLOWED, clientRevision=2
):
...
def hSamrEnumerateDomainsInSamServer(
    dce, serverHandle, enumerationContext=0, preferedMaximumLength=0xFFFFFFFF
):
...
def hSamrEnumerateGroupsInDomain(
    dce, domainHandle, enumerationContext=0, preferedMaximumLength=0xFFFFFFFF
):
...
def hSamrEnumerateAliasesInDomain(
    dce, domainHandle, enumerationContext=0, preferedMaximumLength=0xFFFFFFFF
):
...
def hSamrEnumerateUsersInDomain(
    dce,
    domainHandle,
...
def hSamrQueryDisplayInformation3(
    dce,
    domainHandle,
    displayInformationClass=DOMAIN_DISPLAY_INFORMATION.DomainDisplayUser,
    index=0,
...
def hSamrQueryDisplayInformation2(
    dce,
    domainHandle,
    displayInformationClass=DOMAIN_DISPLAY_INFORMATION.DomainDisplayUser,
    index=0,
...
def hSamrQueryDisplayInformation(
    dce,
    domainHandle,
    displayInformationClass=DOMAIN_DISPLAY_INFORMATION.DomainDisplayUser,
    index=0,
...
def hSamrGetDisplayEnumerationIndex2(
    dce,
    domainHandle,
    displayInformationClass=DOMAIN_DISPLAY_INFORMATION.DomainDisplayUser,
    prefix="",
...
def hSamrGetDisplayEnumerationIndex(
    dce,
    domainHandle,
    displayInformationClass=DOMAIN_DISPLAY_INFORMATION.DomainDisplayUser,
    prefix="",
...
def hSamrCreateAliasInDomain(
    dce, domainHandle, accountName, desiredAccess=GROUP_ALL_ACCESS
):
...
def hSamrQueryInformationDomain(
    dce,
    domainHandle,
    domainInformationClass=DOMAIN_INFORMATION_CLASS.DomainGeneralInformation2,
):
...
def hSamrQueryInformationDomain2(
    dce,
    domainHandle,
    domainInformationClass=DOMAIN_INFORMATION_CLASS.DomainGeneralInformation2,
):
...
def hSamrQueryInformationGroup(
    dce,
    groupHandle,
    groupInformationClass=GROUP_INFORMATION_CLASS.GroupGeneralInformation,
):
...
def hSamrQueryInformationAlias(
    dce,
    aliasHandle,
    aliasInformationClass=ALIAS_INFORMATION_CLASS.AliasGeneralInformation,
):
...
def hSamrQueryInformationUser2(
    dce, userHandle, userInformationClass=USER_INFORMATION_CLASS.UserGeneralInformation
):
...
def hSamrChangePasswordUser(dce, userHandle, oldPassword, newPassword):
    ...
    request["NtPresent"] = 1
    request["OldNtEncryptedWithNewNt"] = crypto.SamEncryptNTLMHash(
        oldPwdHashNT, newPwdHashNT
    )
    request["NewNtEncryptedWithOldNt"] = crypto.SamEncryptNTLMHash(
        newPwdHashNT, oldPwdHashNT
    )
    request["NtCrossEncryptionPresent"] = 0
    request["NewNtEncryptedWithNewLm"] = NULL
    request["LmCrossEncryptionPresent"] = 1
    request["NewLmEncryptedWithNewNt"] = crypto.SamEncryptNTLMHash(
...
def hSamrUnicodeChangePasswordUser2(
    ...
    try:
        samUser["Buffer"] = b"A" * (512 - len(newPassword) * 2) + newPassword.encode(
            "utf-16le"
        )
    except UnicodeDecodeError:
        import sys

        samUser["Buffer"] = b"A" * (512 - len(newPassword) * 2) + newPassword.decode(
    ...
    request["OldNtOwfPasswordEncryptedWithNewNt"] = crypto.SamEncryptNTLMHash(
        oldPwdHashNT, newPwdHashNT
    )
    request["LmPresent"] = 0
...
def hSamrSetPasswordInternal4New(dce, userHandle, password):
    ...
    request["Buffer"]["Internal4New"]["I1"]["SecurityDescriptor"][
        "SecurityDescriptor"
    ] = NULL
