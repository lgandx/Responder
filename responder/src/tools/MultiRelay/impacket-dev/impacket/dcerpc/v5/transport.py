...
class DCERPCStringBinding:
    parser = re.compile(
        r"(?:([a-fA-F0-9-]{8}(?:-[a-fA-F0-9-]{4}){3}-[a-fA-F0-9-]{12})@)?"  # UUID (opt.)
    ...
    def __str__(self):
        return DCERPCStringBindingCompose(
            self.__uuid, self.__ps, self.__na, self.__endpoint, self.__options
        )


def DCERPCStringBindingCompose(
    uuid=None, protocol_sequence="", network_address="", endpoint="", options={}
):
    s = ""
    if uuid:
        s += uuid + "@"
    ...
    if endpoint or options:
        ...
        if options:
            s += "," + ",".join(
                [
                    key if str(val) == "" else "=".join([key, str(val)])
    ...
    def connect(self):
        try:
            af, socktype, proto, canonname, sa = socket.getaddrinfo(
                self.getRemoteHost(), self.get_dport(), 0, socket.SOCK_DGRAM
            )[0]
...
class TCPTransport(DCERPCTransport):
    ...
    def connect(self):
        af, socktype, proto, canonname, sa = socket.getaddrinfo(
            self.getRemoteHost(), self.get_dport(), 0, socket.SOCK_STREAM
        )[0]
        self.__socket = socket.socket(af, socktype, proto)
        try:
            self.__socket.settimeout(self.get_connect_timeout())
...
class HTTPTransport(TCPTransport, RPCProxyClient):
    ...
    def set_credentials(
        ...
        return self._transport.set_credentials(
        ...
        if self._useRpcProxy == False:
            ...
            if data != b"ncacn_http/1.0":
                raise DCERPCException(
                    "%s:%s service is not ncacn_http"
                    % (self.__remoteName, self.__dstport)
                )
        else:
...
class SMBTransport(DCERPCTransport):
    ...
    def __init__(
        ...
        self.set_credentials(
            username, password, domain, lmhash, nthash, aesKey, TGT, TGS
        )
    ...
    def send(self, data, forceWriteAndx=0, forceRecv=0):
        if self._max_send_frag:
            ...
            while 1:
                ...
                if not toSend:
                    break
                self.__smb_connection.writeFile(
                    self.__tid, self.__handle, toSend, offset=offset
                )
                offset += len(toSend)
        else:
